...
class MessageProcessor {
    constructor({ logger, fileExtensions, graphqlFileExtensions, loadConfigOptions, config, parser, tmpDir, connection, }) {
        ...
        this._tmpDir = tmpDir || os_1.tmpdir();
        this._tmpDirBase = path.join(this._tmpDir, 'graphql-language-service');
        this._tmpUriBase = url_1.pathToFileURL(this._tmpDirBase).toString();
        ...
        if (!fs_1.existsSync(this._tmpDirBase)) {
            mkdirp_1.default(this._tmpDirBase);
        }
    }
    get connection() {
    ...
    async _updateGraphQLConfig() {
        ...
        this._graphQLCache = await GraphQLCache_1.getGraphQLCache({
            parser: this._parser,
            loadConfigOptions: this._loadConfigOptions,
            config: this._graphQLConfig,
        });
        this._languageService = new graphql_language_service_1.GraphQLLanguageService(this._graphQLCache);
    ...
    async handleWatchedFilesChangedNotification(params) {
        ...
        return Promise.all(params.changes.map(async (change) => {
            ...
            if (change.type === graphql_language_service_1.FileChangeTypeKind.Created ||
                ...
                const text = fs_1.readFileSync(url_1.fileURLToPath(uri), { encoding: 'utf8' });
            ...
            else if (change.type === graphql_language_service_1.FileChangeTypeKind.Deleted) {
        ...
        const formatted = result
            ? result.definitions.map(res => {
                ...
                if (parentRange && res.name) {
                    ...
                    const isEmbedded = parseDocument_1.DEFAULT_SUPPORTED_EXTENSIONS.includes(path.extname(textDocument.uri));
    ...
    async _cacheSchemaFile(uri, project) {
        uri = uri.toString();
        const isFileUri = fs_1.existsSync(uri);
        let version = 1;
        if (isFileUri) {
            const schemaUri = url_1.pathToFileURL(path.join(project.dirpath, uri)).toString();
            const schemaDocument = this._getCachedDocument(schemaUri);
            if (schemaDocument) {
                version = schemaDocument.version++;
            }
            const schemaText = fs_1.readFileSync(uri, { encoding: 'utf-8' });
    ...
    _getTmpProjectPath(project, prependWithProtocol = true, appendPath) {
        ...
        if (!fs_1.existsSync(projectTmpPath)) {
            mkdirp_1.default(projectTmpPath);
        }
        if (appendPath) {
            projectTmpPath = path.join(projectTmpPath, appendPath);
        }
        if (prependWithProtocol) {
            return url_1.pathToFileURL(path.resolve(projectTmpPath)).toString();
        }
        else {
    ...
    async _cacheConfigSchema(project) {
        try {
            ...
            if (schema) {
                let schemaText = graphql_1.printSchema(schema, {
                ...
                if (cachedSchemaDoc) {
                    fs_1.writeFileSync(fsPath, schemaText, {
                        encoding: 'utf-8',
                    });
    ...
    async _cacheDocumentFilesforProject(project) {
        try {
            ...
            return Promise.all(documents.map(async (document) => {
                ...
                const uri = url_1.pathToFileURL(filePath).toString();
